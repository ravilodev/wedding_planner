...
(function () {
  ...
  function renderDashboard() {
    ...
    $('#app-user-name').textContent = plan.groom || 'Tamu';
    $('#app-user-avatar').textContent = (plan.groom || '?').charAt(0).toUpperCase();
  ...
  function renderAllocation() {
    ...
    if (typeof Chart === 'undefined') {
      console.error('Chart.js belum termuat — skip render grafik, lanjut render bagian lain.');
      showToast('Grafik alokasi gagal dimuat (CDN bermasalah), tapi data lain tetap tampil.', 'warning');
      return;
    }
  ...
  function initNavigation() {
    ...
    $('#header-cta').addEventListener('click', () => {
      if ($('#view-app').classList.contains('hidden')) showView('wizard');
      else confirmRestart();
    });
    $('#restart-btn').addEventListener('click', confirmRestart);
    $('#nav-dashboard').addEventListener('click', (e) => { e.preventDefault(); showView('app'); });
  ...
  /* ------------------------------------------------------------------
     10b. APP SHELL (sidebar navigation, page switching, mobile drawer)
  ------------------------------------------------------------------ */
  function switchAppPage(page) {
    $$('.app-page').forEach(p => p.classList.toggle('active', p.dataset.pagePanel === page));
    $$('.app-nav-item[data-page]').forEach(b => b.classList.toggle('active', b.dataset.page === page));
    $$('.app-bottom-item[data-page]').forEach(b => b.classList.toggle('active', b.dataset.page === page));
    closeMobileSidebar();
  }

  function openMobileSidebar() {
    $('#app-sidebar').classList.add('mobile-open');
    $('#app-sidebar-backdrop').classList.add('open');
  }
  function closeMobileSidebar() {
    $('#app-sidebar').classList.remove('mobile-open');
    $('#app-sidebar-backdrop').classList.remove('open');
  }

  function initAppShell() {
    $$('.app-nav-item[data-page]').forEach(btn => btn.addEventListener('click', () => switchAppPage(btn.dataset.page)));
    $$('.app-bottom-item[data-page]').forEach(btn => btn.addEventListener('click', () => switchAppPage(btn.dataset.page)));

    // "AI Assistant" in the sidebar opens the existing slide-out panel rather than a page.
    $('#nav-ai-assistant').addEventListener('click', () => { closeMobileSidebar(); $('#ai-fab').click(); });

    // "Lainnya" on mobile bottom nav opens the full sidebar drawer (Timeline, Ide, Dokumen, dst. live there).
    $('#bottom-nav-more').addEventListener('click', openMobileSidebar);

    $('#sidebar-collapse-btn').addEventListener('click', () => $('#app-sidebar').classList.toggle('collapsed'));
    $('#mobile-menu-btn').addEventListener('click', openMobileSidebar);
    $('#app-sidebar-backdrop').addEventListener('click', closeMobileSidebar);

    // Nikara Premium upsell — subscription/payment not wired up yet (see roadmap Fase 4).
    $('.app-premium-btn').addEventListener('click', () => {
      showToast('Nikara Premium akan segera hadir — fitur pembayaran belum aktif.', 'warning');
    });
  }
